...
public class AdminController {
	...
	public ResponseEntity<GetClassRoomResponseDto> getClassroom(@PathVariable long aadharNumber){
		try {
			...
			List<StudentClassroom> classrooms = studentClassroomRepository.findByStudentIdAndCurrentDate(students.get(0).getId());
		...
		try {
			...
			while(subjectNumber < examSubjects.size()) {
				...
				while(studentNumber < students.size()) {
					...
					int subject = examSubjects.get(subjectNumber).getId();
					int student = students.get(studentNumber).getStudentId();
					
					List<StudentClassroom> existingStudentClassroom = 
							studentClassroomRepository.findByStudentIdAndExamSubjectId(student, subject);
					
					if (existingStudentClassroom.size() > 0) {
						studentNumber++;
						continue;
					}
					...
					existingStudentClassroom = 
							studentClassroomRepository.findByClassroomIdAndExamSubjectId(classroom.getId(), subject);
					
					if (existingStudentClassroom.size() > 0 || classroom.getCapacity() == numberOfStudents) {
						classroomNumber++;
						continue;
					}
					...
					studentClassroom.setExamSubjectId(subject);
